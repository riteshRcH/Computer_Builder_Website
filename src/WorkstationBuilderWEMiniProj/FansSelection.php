<?php
	...
	$_SESSION['StorageID'] = $_REQUEST['StorageID'];
?>
...
<head>
	...
	<script>
		...
		{
			var radios = document.forms["selectFanForm"]["FanID"];
			if(!isArray(radios))
				return true;						//because i have already CHECKED it using dynamic PHP code, see line 98
			for(var i=0;i<radios.length;i++)
				if(radios[i].checked)
					return true;
			alert("Select a primary CPU Cooling Fan 1st");
			return false;
		}
		function isArray(h)
		{
...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectFanForm" method="get" action="SMPSSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from fans");
		...
		$result = mysql_query("select * from fans");			//static because no Laptop Mobos present in motherboard table
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="FanID" value="'.$row['FanID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';

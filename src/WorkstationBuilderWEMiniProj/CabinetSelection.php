<?php
	...
	$_SESSION['MonitorID'] = $_REQUEST['MonitorID'];
?>
...
<head>
	...
	<script>
		...
		{
			var radios = document.forms["selectCabinetForm"]["CabinetID"];
			if(!isArray(radios))
				return true;						//because i have already CHECKED it using dynamic PHP code, see line 98
			for(var i=0;i<radios.length;i++)
				if(radios[i].checked)
					return true;
			alert("Cabinet determines the looks, cooling efficiency and form factor compatability issues .. its vital to choose the best cooling cabinet!");
			return false;
		}
		function isArray(h)
		{
			return (h.length!=undefined&&h[0]!=undefined)||(h.length===0&&h[0]===undefined);
		}
	</script>
</head>
...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectCabinetForm" method="get" action="DoneWithSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from cabinets");
		...
		$result = mysql_query("select * from cabinets");			//static because no Laptop Mobos present in motherboard table
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="CabinetID" value="'.$row['CabinetID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';

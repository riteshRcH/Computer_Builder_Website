...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectMoboForm" method="get" action="RAMSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from motherboard");
		...
		$result = mysql_query("select * from motherboard where CPUSocketSupport IN (select SocketType from cpu where SocketType=(select SocketType from cpu where CPUID=".$_SESSION['CPUID']."))");
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="MoboID" value="'.$row['MoboID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';
			for($index = 1; $index < count($columnNames); $index += 1)
			{
				if($columnNames[$index]=="UEFIBIOSPResent")
				{

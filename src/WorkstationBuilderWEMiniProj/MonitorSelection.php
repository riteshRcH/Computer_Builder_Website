...
<head>
	...
	<script>
		...
		{
			var radios = document.forms["selectMonitorForm"]["MonitorID"];
			if(!isArray(radios))
...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectMonitorForm" method="get" action="CabinetSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from monitor");
		...
		$result = mysql_query("select * from monitor");			//static because no Laptop Mobos present in motherboard table
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="MonitorID" value="'.$row['MonitorID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';
			for($index = 1; $index < count($columnNames); $index += 1)
			{
				if($columnNames[$index]=="3DSupport" || $columnNames[$index]=="HDSupport" || $columnNames[$index]=="DVI" || $columnNames[$index]=="HDMI" || $columnNames[$index]=="VGA")
				{

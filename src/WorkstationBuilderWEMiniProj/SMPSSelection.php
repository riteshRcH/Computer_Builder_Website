...
<head>
	...
	<title>Select Cooling Fan</title>
	...
	<script>
		...
		{
			var radios = document.forms["selectSMPSForm"]["SMPSID"];
			if(!isArray(radios))
				return true;						//because i have already CHECKED it using dynamic PHP code, see line 98
			for(var i=0;i<radios.length;i++)
				if(radios[i].checked)
					return true;
			alert("Without an SMPS, a computer cant run .. selecting a good SMPS is vital");
			return false;
		}
		function isArray(h)
		{
...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectSMPSForm" method="get" action="KeyboardsSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from smps");
		...
		$result = mysql_query("select * from smps");			//static because no Laptop Mobos present in motherboard table
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="SMPSID" value="'.$row['SMPSID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';
			for($index = 1; $index < count($columnNames); $index += 1)
			{
				if($columnNames[$index]=="SLICrossFireReady" || $columnNames[$index]=="HaswellReady")
				{

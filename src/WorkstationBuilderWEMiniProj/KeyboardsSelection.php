<?php
	...
	$_SESSION['SMPSID'] = $_REQUEST['SMPSID'];
?>
...
<head>
	...
	<title>Select your gaming Keyboard</title>
...
<div id="outerDivision">
	...
	<?php
		...
		echo '<form name="selectKBForm" method="get" action="MonitorSelection.php" onsubmit="return validateChoice()"><center><input type="submit" value="Next" style="color: red; width: 90px; height: 40px;"/></center><br /><br />';
		echo '<table border="5" cellpadding="5" cellspacing="11">';
		$result = mysql_query("show columns from keyboards");
		...
		$result = mysql_query("select * from keyboards");			//static because no Laptop Mobos present in motherboard table
		while($row = mysql_fetch_array($result))
		{
			echo '<td><input type="radio" name="KBID" value="'.$row['KBID'].'" '.(mysql_num_rows($result)==1?"CHECKED":"").'/></td>';
			for($index = 1; $index < count($columnNames); $index += 1)
			{
				if($columnNames[$index]=="HasMultimediaKeys")
				{
